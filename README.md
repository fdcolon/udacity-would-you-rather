...
If you select the `Ghost User` and click on the `Sign In` button, you will get an error message saying that the user is invalid.
...
Be aware that once you are logged in, you should use only the `Navigation Bar` to navigate through the app. You shouldn't type any URL into the browser's address bar or you will be logged out.
...
**Figure 21** - Home Page (new question added)
...
If you created a new user, you should be able to see it into the users list in the `Login` page. If you would like to sign in using his/her account, just select the new user and click on the `Sign In` button.
...
> :warning: If you provide a valid path to see the details of an exisgint poll, you will be redirected to the `Login` page. This is because remember that everytime you provide a `URL` from the address bar, `Redux` will reset all the `store` values. To test it, you can try the following URL.
```
